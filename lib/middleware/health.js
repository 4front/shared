...
module.exports = function(settings, apiRouter, hostRouter) {
  ...
  router.get('/full', function(req, res, next) {
    ...
    function createApplication(callback) {
      ...
      var params = {
        url: '/orgs/' + testOrg.orgId + '/apps',
        method: 'post',
        body: {name: appName}
      };

      makeApiCall(params, function(err, app) {
        if (err) return callback(err);
        testApp = app;
        callback();
      });
    ...
    function createVersion(callback) {
      ...
      var params = {url: '/apps/' + testApp.appId + '/versions', body: postData, method: 'post'};
      makeApiCall(params, function(err, version) {
        if (err) return callback(err);
        testVersion = version;
        callback();
      });
    ...
    function deployIndexHtml(callback) {
      ...
      var apiOptions = {
        ...
        body: '<html><head></head><body>' +
          testApp.appId + ',' + testVersion.versionId + '</body></html>'
      };
    ...
    function curlAppIndexUrl(callback) {
      ...
      supertest(apphost).get('/')
        ...
        .expect(function(resp) {
          ...
          assert.ok(resp.text.indexOf('<body>' + testApp.appId + ','
            + testVersion.versionId + '</body>') > 0);
        })
        .end(callback);
    }
