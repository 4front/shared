...
module.exports = function(options) {
  ...
  options = _.defaults(options || {}, {
    ...
    // match a line in a core node file. Key is the leading
    // open paren "(main.js" without a file path.
    coreFileRe: /\([a-z\-_0-9]+\.js/,
    nodeModulesWhitelist: [/^4front\-/, /htmlprep/, /express-request-proxy/],
    prettyLogJson: false,
    // match a line in a core node file. Key is the leading open
    // paren "(main.js" without a file path.
    transports: [
  ...
  return {
    ...
    error: function(err) {
      ...
      if (errorJson) {
        // Ensure the stack is an array
        if (_.isString(errorJson.stack)) {
          errorJson.stack = errorJson.stack.split('\n');
        }
        if (_.isArray(errorJson.stack)) {
          errorJson.stack = _.compact(errorJson.stack);
        }
  ...
  function formatStack(stack) {
    ...
    for (var i = 0; i < originalLines.length; i++) {
      ...
      if (nodeModulesMatch) {
        ...
        // nodeModulesMatch.
        if (line.substr(line.indexOf(nodeModulesMatch[0]) +
          nodeModulesMatch[0].length, 12) === 'node_modules') {
          continue;
        }
      }
  ...
  function stringifyLogEntry(obj) {
    // Omit keys from log object where value is empty string or null.
    var logObject = _.omitBy(obj, function(value) {
      return (_.isString(value) && value.length === 0) || value === null;
    });
